...
.app-header { background: linear-gradient(135deg, #1C1C1E 0%, #0F0F12 100%); border-radius: 20px; padding: 20px; margin-bottom: 20px; text-align: center; border: 1px solid #2C2C2E; }
.app-header h1 { font-size: 22px !important; margin: 0 !important; color: #FFFFFF !important; }
...
.label { font-size: 10px; color: #8E8E93; text-transform: uppercase; font-weight: 700; margin-bottom: 4px; }
.score-box { background: linear-gradient(135deg, #007AFF 0%, #5856D6 100%); border-radius: 12px; padding: 12px; text-align: center; margin: 10px 0; }
.score-val { font-size: 26px; font-weight: 900; color: white; }
.bruce-avis { background: #1C1C1E; border-radius: 16px; padding: 15px; border-left: 4px solid #007AFF; margin-top: 15px; }
...
div.stButton > button:hover { background-color: #007AFF !important; }
</style>
...
def main():
    ...
    if search_query:
        ...
        if suggestions:
            st.markdown("<p style='font-size:12px; color:#8E8E93;'>Résultats trouvés :</p>", unsafe_allow_html=True)
    ...
    try:
        ...
        fig = go.Figure(data=[go.Scatter(x=hist.index, y=hist['Close'], line=dict(color='#007AFF', width=3))])
